...
for f = 1:file_count
    ...
    ui_data = squeeze(mean(all_data{f}, 3, 'omitnan'));  % [lon, lat]
    % Add to monthly climatology
    monthly_climatology(:, :, m) = monthly_climatology(:, :, m) + ui_data;
...
for m = 1:12
    if monthly_count(m) > 0
        monthly_climatology(:, :, m) = monthly_climatology(:, :, m) / monthly_count(m);
    else
