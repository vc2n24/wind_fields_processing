...
for y= 1:length(years_numbers)
    ...
    for m = 1:length(months)
       ...
       % Prepare output file path 
        ...
        out_file = sprintf('CCMP_products_monthly_%s%s%s', Ynumber, Mnumber, '_V03.1_L4.nc');
        out_path = fullfile(out_dir, out_file);

 % Create variables in the new file 
            nccreate(out_path, 'u_along', 'Dimensions', {'longitude', 192, 'latitude', 128, 'time', 1}, 'Datatype','single', 'FillValue', single(-9999));
            nccreate(out_path, 'u_cross', 'Dimensions', {'longitude', 192, 'latitude', 128, 'time', 1}, 'Datatype','single', 'FillValue', single(-9999));
            nccreate(out_path, 'taux', 'Dimensions', {'longitude', 192, 'latitude', 128, 'time', 1}, 'Datatype','single', 'FillValue', single(-9999));
            nccreate(out_path, 'tauy', 'Dimensions', {'longitude', 192, 'latitude', 128, 'time', 1}, 'Datatype','single', 'FillValue', single(-9999));
            nccreate(out_path, 'tau_along', 'Dimensions', {'longitude', 192, 'latitude', 128, 'time', 1}, 'Datatype','single', 'FillValue', single(-9999));
            nccreate(out_path, 'tau_cross', 'Dimensions', {'longitude', 192, 'latitude', 128, 'time', 1}, 'Datatype','single', 'FillValue', single(-9999));
            ...
            ncwriteatt(out_path, '/', 'script_used', 'ccmp_monthly_products_script.m');
